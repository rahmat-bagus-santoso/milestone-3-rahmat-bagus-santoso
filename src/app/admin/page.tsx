    ...
    <div className="min-h-screen bg-zinc-50 flex">
      ...
      <main className="flex-1 p-12 overflow-y-auto">
        <div className="flex justify-between items-end mb-12">
          ...
          <button onClick={handleCreate} className="bg-zinc-900 text-white px-6 py-2 text-[10px] font-black uppercase tracking-widest">Add Product</button>
        </div>
            ...
            <tbody className="divide-y divide-zinc-100">
              {products.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="p-4 text-red-500"><button onClick={() => handleDelete(product.id.toString())}>Delete</button></td>
                </tr>
              ))}
            </tbody>
          </table>
